...
export function createMonoClassTests(): TestResult[] {
  ...
  results.push(createMonoDependentTest(
    "MonoClass should handle generic List<T> type discovery",
    () => {
      const domain = Mono.domain;
  ...
  results.push(createMonoDependentTest(
    "MonoClass should handle generic Dictionary<K,V> type discovery",
    () => {
      const domain = Mono.domain;
  ...
  results.push(createMonoDependentTest(
    "MonoClass should enumerate nested types",
    () => {
      const domain = Mono.domain;
      ...
      if (environmentClass) {
        ...
        nestedTypes.forEach((nested, i) => {
          if (i < 3) { // Log first 3 nested types
  ...
  results.push(createMonoDependentTest(
    "MonoClass should handle nested class lookup by name pattern",
    () => {
      const domain = Mono.domain;
      
      // Try common nested type patterns
      const assemblyNames = domain.assemblies.map(a => a.name);
      const hasUserAssembly = assemblyNames.some(n => 
        n.includes("Assembly-CSharp") || n.includes("Game")
      );
  ...
  results.push(createMonoDependentTest(
    "MonoClass should identify value types correctly",
    () => {
      const domain = Mono.domain;
  ...
  results.push(createMonoDependentTest(
    "MonoClass should identify enum types correctly",
    () => {
      const domain = Mono.domain;
  ...
  results.push(createMonoDependentTest(
    "MonoClass should identify delegate types correctly",
    () => {
      const domain = Mono.domain;
  ...
  results.push(createMonoDependentTest(
    "MonoClass should enumerate implemented interfaces",
    () => {
      const domain = Mono.domain;
      const stringClass = domain.class("System.String");
  ...
  results.push(createMonoDependentTest(
    "MonoClass should check interface implementation correctly",
    () => {
      const domain = Mono.domain;
      const stringClass = domain.class("System.String");
      const listClass = domain.class("System.Collections.Generic.List`1");
  ...
  results.push(createMonoDependentTest(
    "MonoClass should traverse full type hierarchy",
    () => {
      const domain = Mono.domain;
  ...
  results.push(createErrorHandlingTest(
    "MonoClass should handle whitespace-only class names",
    () => {
      const domain = Mono.domain;
  ...
  results.push(createErrorHandlingTest(
    "MonoClass should handle special characters in class names",
    () => {
      const domain = Mono.domain;
  ...
  results.push(createMonoDependentTest(
    "MonoClass should get VTable correctly",
    () => {
      const domain = Mono.domain;
      const stringClass = domain.class("System.String");
  ...
  results.push(createMonoDependentTest(
    "MonoClass should ensure initialization before VTable access",
    () => {
      const domain = Mono.domain;
      const objectClass = domain.class("System.Object");
  ...
  results.push(createMonoDependentTest(
    "MonoClass should provide complete describe() output",
    () => {
      const domain = Mono.domain;
      const stringClass = domain.class("System.String");
      ...
      console.log(`    - Methods: ${summary.methodCount}, Fields: ${summary.fieldCount}, Properties: ${summary.propertyCount}`);
    }
  ));
