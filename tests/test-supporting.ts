...
export function testSupporting(): TestResult {
  ...
  suite.addResult(
    createStandaloneTest("MonoEnums should be immutable", () => {
  ...
  suite.addResult(
    createMonoDependentTest("Should test profiler and debugger defines", () => {
      ...
      console.log(
        `    Debugger version: ${MonoDefines.MONO_DEBUGGER_MAJOR_VERSION}.${MonoDefines.MONO_DEBUGGER_MINOR_VERSION}`,
      );
    }),
  );
  ...
  suite.addResult(
    createMonoDependentTest("Logger works with Mono domain operations", () => {
      ...
      if (version) {
        logger.info(
          `Features: delegateThunk=${version.features.delegateThunk}, metadataTables=${version.features.metadataTables}`,
        );
      }
    }),
  );
  ...
  suite.addResult(
    createDomainTest("Should test cross-feature supporting operations", domain => {
      ...
      if (version) {
        logger.info(
          `Version features: delegateThunk=${version.features.delegateThunk}, gcHandles=${version.features.gcHandles}`,
        );
      }
