 ...
 */
export function createUnityAssemblyTest(
  assemblyName: string,
 ...
 */
export function createUnityNamespaceTest(
  namespace: string,
...
export function createPerformanceTest(
  ...
  return createTest(testName, () => {
    ...
    if (mergedOptions.memoryThreshold && memoryDelta > mergedOptions.memoryThreshold) {
      errors.push(`Memory delta ${(memoryDelta / 1024 / 1024).toFixed(2)}MB exceeds threshold ${(mergedOptions.memoryThreshold / 1024 / 1024).toFixed(2)}MB`);
    }
...
export function createMethodLookupPerformanceTest(
  ...
  return createBasicLookupPerformanceTest(
    `Method lookup performance for ${className}.${methodName}`,
    () => {
 ...
 */
export function createFieldLookupPerformanceTest(
  className: string,
  fieldName: string
): TestResult {
  return createBasicLookupPerformanceTest(
    `Field lookup performance for ${className}.${fieldName}`,
    () => {
      const domain = Mono.domain;
      const targetClass = domain.class(className);
      if (targetClass) {
 ...
 */
export function createPropertyLookupPerformanceTest(
  className: string,
  propertyName: string
): TestResult {
  return createBasicLookupPerformanceTest(
    `Property lookup performance for ${className}.${propertyName}`,
    () => {
      const domain = Mono.domain;
      const targetClass = domain.class(className);
      if (targetClass) {
...
export function createBasicStringCreationTest(): TestResult {
  return createMonoDependentTest(
    "String creation should work correctly",
    () => {
      const testCases = [
        "Hello World",
        "",
...
export function createStringManipulationTest(): TestResult {
  return createMonoDependentTest(
    "Should test string manipulation operations",
    () => {
...
export function createStringEncodingTest(): TestResult {
  return createMonoDependentTest(
    "Should test string encoding and UTF handling",
    () => {
      const encodingTests = [
        "ASCII only",
...
export function createStringComparisonTest(): TestResult {
  return createMonoDependentTest(
    "Should test string comparison and searching",
    () => {
      const testStrings = [
        "Hello World",
...
export function createInvalidNameTests(
  ...
  results.push(createErrorHandlingTest(
    `${testType} should handle empty names gracefully`,
    () => {
  ...
  results.push(createErrorHandlingTest(
    `${testType} should handle null names gracefully`,
    () => {
      try {
  ...
  invalidNames.forEach((invalidName, index) => {
    results.push(createErrorHandlingTest(
      `${testType} should handle invalid name ${index + 1}`,
      () => {
...
export function createRequiredAccessTests(
  ...
  results.push(createErrorHandlingTest(
    `${testType} should throw for empty required names`,
    () => {
      assertThrows(() => {
  ...
  results.push(createErrorHandlingTest(
    `${testType} should throw for null required names`,
    () => {
      assertThrows(() => {
  ...
  invalidNames.forEach((invalidName, index) => {
    results.push(createErrorHandlingTest(
      `${testType} should throw for invalid required name ${index + 1}`,
      () => {
        assertThrows(() => {
...
export function createBoundaryValueTests(
  ...
  boundaryValues.forEach((value, index) => {
    ...
    results.push(createMonoDependentTest(description, () => {
      try {
        testFunction(value);
