...
Mono.perform(() => {
  ...
  console.log('Generic arguments:', args?.map(a => a.getFullName()));
  ...
  if (dictDef && stringClass && intClass) {
    ...
    console.log('Generic arguments:', dictOfStringInt?.getGenericArguments().map(a => a.getFullName()));
  }
  ...
  if (arrayClass) {
    ...
    const emptyMethod = methods.find(m => 
      m.getName() === 'Empty' && m.isGenericMethodDefinition()
    );
