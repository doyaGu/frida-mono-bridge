...
export function createFindToolTests(): TestResult[] {
  ...
  results.push(createMonoDependentTest(
    'Find - classExact function exists',
    () => {
  ...
  // =====================================================
  results.push(createMonoDependentTest(
    'Find.classExact - find System.String',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.classExact - find System.Int32',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.classExact - find System.Object',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.classExact - non-existent class returns null',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.classExact - find class without namespace',
    () => {
  ...
  // =====================================================
  results.push(createMonoDependentTest(
    'Find.classes - search with * wildcard',
    () => {
      ...
      const hasSystemString = stringClasses.some(
        c => c.getFullName() === 'System.String'
      );
  ...
  results.push(createMonoDependentTest(
    'Find.classes - search System.* namespace',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.classes - search class name only without namespace',
    () => {
      ...
      for (const klass of classes) {
        ...
        assert(
          ...
          `Class name ${name} should contain 'object'`
        );
      }
  ...
  results.push(createMonoDependentTest(
    'Find.classes - use ? single character wildcard',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.classes - empty pattern returns all classes',
    () => {
      // * should match all
  ...
  // =====================================================
  results.push(createMonoDependentTest(
    'Find.methods - search ToString methods',
    () => {
      ...
      for (const method of methods.slice(0, 5)) {
        ...
        assert(
          name.toLowerCase().includes('tostring'),
          `Method name ${name} should contain 'toString'`
        );
      }
  ...
  results.push(createMonoDependentTest(
    'Find.methods - search ClassName.MethodName format',
    () => {
      ...
      for (const method of methods.slice(0, 5)) {
        ...
        assert(
          className === 'String',
          `Method should belong to String class, got ${className}`
        );
      }
  ...
  results.push(createMonoDependentTest(
    'Find.methods - search Get* methods',
    () => {
  ...
  // =====================================================
  results.push(createMonoDependentTest(
    'Find.fields - search Empty field',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.fields - search ClassName.FieldName format',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.fields - search _* private field pattern',
    () => {
  ...
  // =====================================================
  results.push(createMonoDependentTest(
    'Find.classes - case insensitive search',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.methods - case insensitive search',
    () => {
  ...
  // =====================================================
  results.push(createMonoDependentTest(
    'Find.classes - search UnityEngine.* classes',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.methods - search Update methods (Unity)',
    () => {
  ...
  // =====================================================
  results.push(createMonoDependentTest(
    'Find.classes - empty string pattern',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.classes - special character patterns',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.methods - non-existent pattern',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.fields - non-existent pattern',
    () => {
  ...
  // =====================================================
  results.push(createMonoDependentTest(
    'Find.classes - performance: search all classes',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.methods - performance: search common methods',
    () => {
      const startTime = Date.now();
  ...
  // =====================================================
  results.push(createMonoDependentTest(
    'Find.classes - results are valid MonoClass objects',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.methods - results are valid MonoMethod objects',
    () => {
      const methods = Mono.find.methods(Mono.api, '*ToString*');
  ...
  results.push(createMonoDependentTest(
    'Find.fields - results are valid MonoField objects',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.classes - regex search - starting with uppercase',
    () => {
  ...
  results.push(createMonoDependentTest(
    ...
    () => {
      ...
      for (const method of methods.slice(0, 10)) {
        const name = method.getName();
        assert(name.startsWith('get_') || name.startsWith('set_'), 
  ...
  // =====================================================
  results.push(createMonoDependentTest(
    'Find.classes - filter option for interfaces',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.methods - filter option for static methods',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.fields - filter option for static fields',
    () => {
      const fields = Mono.find.fields(Mono.api, '*', { 
  ...
  // =====================================================
  results.push(createMonoDependentTest(
    'Find.properties - function exists',
    () => {
  ...
  results.push(createMonoDependentTest(
    'Find.properties - ClassName.PropertyName format',
    () => {
      const props = Mono.find.properties(Mono.api, 'System.String.*', { limit: 20 });
      
      for (const prop of props.slice(0, 5)) {
        const parent = prop.getParent();
        assert(parent.getNamespace() === 'System' && parent.getName() === 'String',
  ...
  results.push(createMonoDependentTest(
    'Find.properties - results are valid MonoProperty objects',
    () => {
