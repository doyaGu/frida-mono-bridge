...
export function testUnityEngineModules(): TestResult {
  ...
  // Math operations
  suite.addResult(
    createMonoDependentTest("Math operations should be available", () => {
      const domain = Mono.domain;
  ...
  suite.addResult(
    createMonoDependentTest("Physics operations should be available", () => {
      ...
      console.log(
        `    Physics.defaultContactOffset: ${defaultContactOffsetProperty !== null ? "Available" : "Not available"}`,
      );
  ...
  suite.addResult(
    createErrorHandlingTest("Engine module operations should handle errors gracefully", () => {
      const domain = Mono.domain;
      const vector3Class = domain.class("UnityEngine.Vector3");
  ...
  suite.addResult(
    createPerformanceTest("Engine module operations performance", () => {
      const domain = Mono.domain;
      const vector3Class = domain.class("UnityEngine.Vector3");
