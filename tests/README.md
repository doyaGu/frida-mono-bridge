...
Comprehensive test suite for frida-mono-bridge with **1,104 tests** across **36 test files**.
...
| [test-find-tools.ts](test-find-tools.ts) | 20 | Advanced Features |
...
| File | Tests | Description |
|------|-------|-------------|
| [test-mono-domain.ts](test-mono-domain.ts) | 3 | MonoDomain operations, assembly loading |
...
| File | Tests | Description |
|------|-------|-------------|
| [test-find-tools.ts](test-find-tools.ts) | 20 | `Mono.find.classExact`, fast class lookup |
...
### Find Tools Performance

The `classExact()` function has optimized fast paths:

- **System.*** classes → Direct `mscorlib` lookup (instant)
- **UnityEngine.*** classes → Direct `UnityEngine.CoreModule` lookup (instant)
- Other namespaces → Full assembly enumeration (slow)

```typescript
// Fast (uses optimized path)
Mono.find.classExact("System.String");           // ~0ms
Mono.find.classExact("UnityEngine.GameObject");  // ~0ms

// Slow (enumerates all assemblies)
Mono.find.classes("*MyClass*");                  // May timeout
```
...
runTestCategory("Xxx Tests", async () => {
...
| `npm run test:find-tools` | Find utilities tests |
...
- All `classExact()` tests with System.* or UnityEngine.* namespaces
...
|------|--------|------------|
| `Find.classes("*")` | Enumerates all assemblies | Use `classExact()` |
| `Find.methods("*")` | Enumerates all classes | Use specific class |
...
2. **Find wildcard searches**: Slow in Unity projects. Use `classExact()` or `limit` option.
...
runTestCategory("My Feature Tests", async () => {
