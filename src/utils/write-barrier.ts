 ...
 * ```
 */
export function genericReferenceStoreWithBarrier(api: MonoApi, destination: NativePointer, value: NativePointer): void {
  if (value.isNull()) {
    // NULL writes don't need barriers
 ...
 * @param value Pointer to the new value (managed object reference)
 */
export function atomicReferenceStoreWithBarrier(api: MonoApi, destination: NativePointer, value: NativePointer): void {
  if (value.isNull()) {
 ...
 * ```
 */
export function copyObjectWithBarrier(api: MonoApi, destination: NativePointer, source: NativePointer): void {
